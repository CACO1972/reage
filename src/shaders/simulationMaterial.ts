  ...
  return new ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec3 noiseInput = vec3(x * 2.0 + time * 0.5, y * 2.0 + time * 0.3, time);
        float noise = snoise(noiseInput) * 0.6;
        
        // Apply curl noise for flow
        vec3 curl = curlNoise(vec3(x * 1.5, y * 1.5, time * 0.5)) * uCurl * 2.0;
        
        vec3 pos = basePos + vec3(
          noise + curl.x,
          noise * 0.5 + curl.y,
          noise * 1.5 + curl.z * 2.0
        );
        
        // Add wave motion
        pos.z += sin(x * 6.28 + time * 2.0) * 0.3;
        pos.z += cos(y * 6.28 + time * 1.5) * 0.2;
